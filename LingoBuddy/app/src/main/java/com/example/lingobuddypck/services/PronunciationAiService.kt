...
class PronunciationAiService(
    ...
    suspend fun checkPronunciation(userSpeech: String, referenceText: String): PronunciationFeedback {
        ...
        if (!response.isSuccessful) {
            ...
            throw kotlin.Exception("API call for pronunciation check failed with code ${response.code()}: $errorBody")
        }

        val responseBodyText = response.body()?.output?.choices?.getOrNull(0)?.text
        if (responseBodyText.isNullOrBlank()) {
            throw kotlin.Exception("No response from AI for pronunciation check.")
        }
        ...
        return try {
            ...
            throw kotlin.Exception(
                "Error parsing pronunciation feedback JSON: ${e.message}. Raw: $actualJson",
                e
            )
        }
    }
